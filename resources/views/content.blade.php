    ...
    <div class="flex items-start justify-between mb-10">
        ...
        <div class="flex items-center gap-3">
            <button type="button" onclick="document.getElementById('create-folder').showModal()"
                class="bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 text-sm font-semibold px-5 py-3 rounded-xl transition">
                + Folder
            </button>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
                    @foreach ($folders as $folder)
                        ...
                        <a href="{{ route('content.index', ['folder_id' => $folder->id]) }}"
                            class="group bg-zinc-800/80 border border-zinc-700/50 rounded-2xl p-5 flex flex-col gap-3 hover:border-zinc-600 hover:shadow-xl hover:shadow-black/20 transition">
                            <div class="w-10 h-10 rounded-xl bg-emerald-500/15 flex items-center justify-center group-hover:bg-emerald-500/25 transition">
                                <svg class="w-5 h-5 text-emerald-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    ...
                    @foreach ($notes as $note)
                        <div class="bg-zinc-800/80 border border-zinc-700/50 rounded-2xl p-6 min-h-[140px] flex flex-col gap-3 hover:border-zinc-600 hover:shadow-xl hover:shadow-black/20 transition">
                            <div class="flex items-start justify-between gap-3">
                                <h4 class="text-lg font-semibold text-white">{{ $note->title }}</h4>
                                <span class="shrink-0 text-xs text-zinc-500">{{ $note->created_at->diffForHumans() }}</span>
                            </div>
